...
{
    public static string AgentDir { get; } = Path.Combine(Path.GetTempPath(), "pisharp-tests-" + Guid.NewGuid().ToString("N")[..8], "agent");

    [ModuleInitializer]
    internal static void Initialize()
    {
        Directory.CreateDirectory(Path.Combine(AgentDir, "bin"));
        Environment.SetEnvironmentVariable("PISHARP_CODING_AGENT_DIR", AgentDir);
...
{
    public string Path { get; } = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "pisharp-tool-" + Guid.NewGuid().ToString("N")[..10]);
