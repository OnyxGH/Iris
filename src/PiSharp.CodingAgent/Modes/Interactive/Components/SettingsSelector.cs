...
public sealed class SettingsSelectorComponent : Container, IInputComponent
{
    ...
    public static string FormatHttpIdleTimeoutMs(long timeoutMs) =>
        HttpIdleTimeoutChoices.FirstOrDefault(c => c.TimeoutMs == timeoutMs).Label ?? $"{PiSharp.CodingAgent.Core.Tools.NodeCompat.FormatNumber(timeoutMs / 1000.0)} sec";
                        ...
                        new()
                        {
                            ...
                            Options = _ =>
                            {
                                var sorted = config.AvailableDefaultModels.OrderBy(m => m, Comparer<Model>.Create((a, b) =>
                                {
                                    ...
                                    return PiSharp.Tui.NodeCompare.LocaleCompare(a.Provider, b.Provider);
                                })).ToList();
