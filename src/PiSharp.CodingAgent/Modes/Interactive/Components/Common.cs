...
public sealed class AssistantMessageComponent : Container
{
    ...
    private int _outputPad;
    private readonly IReadOnlyList<MarkdownTransformer> _transformers;
    private PiSharp.Ai.AssistantMessage? _lastMessage;
    private bool _hasToolCalls;
    private bool _isStreaming;

    public AssistantMessageComponent(PiSharp.Ai.AssistantMessage? message = null, bool hideThinkingBlock = false, MarkdownTheme? markdownTheme = null, string hiddenThinkingLabel = "Thinking...", int outputPad = 1, IReadOnlyList<MarkdownTransformer>? transformers = null)
    {
    ...
    private static bool IsVisible(PiSharp.Ai.ContentBlock c) =>
        (c is PiSharp.Ai.TextContent t && t.Text.Trim().Length > 0) || (c is PiSharp.Ai.ThinkingContent th && th.Thinking.Trim().Length > 0);

    public void UpdateContent(PiSharp.Ai.AssistantMessage message, bool? isStreaming = null)
    {
        ...
        {
            if (content[i] is PiSharp.Ai.TextContent text && text.Text.Trim().Length > 0)
            {
            ...
            else if (content[i] is PiSharp.Ai.ThinkingContent)
            {
                var blocks = new List<string>();
                for (; i < content.Count; i++)
                {
                    if (content[i] is not PiSharp.Ai.ThinkingContent thinkingContent) break;
        ...
        _hasToolCalls = content.Any(c => c is PiSharp.Ai.ToolCall);
        if (message.StopReason == PiSharp.Ai.StopReason.Length)
        {
        ...
        {
            if (message.StopReason == PiSharp.Ai.StopReason.Aborted)
            {
            ...
            else if (message.StopReason == PiSharp.Ai.StopReason.Error)
            {
