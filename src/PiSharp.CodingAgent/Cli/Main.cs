...
using PiSharp.CodingAgent.Modes.Interactive;
using PiSharp.CodingAgent.Modes;
...
public static class Main
{
    ...
    private static async Task<int> RunCoreAsync(string[] args)
    {
        ...
        EnvHttpProxy.ApplyHttpProxySetting(PiSharp.Ai.Json.PiJson.GetString(bootstrapSettingsManager.GetGlobalSettings()["httpProxy"]));
        ...
        {
            Console.Error.WriteLine(Chalk.Red("Error: --export is not available in PiSharp yet"));
            return 1;
        }
        ...
        if (appMode == "rpc")
        {
            ...
            if (!offlineMode)
            {
                _ = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        await services.ModelRuntime.RefreshAsync(new PiSharp.Ai.Models.ModelsRefreshOptions { CancellationToken = refreshTimeout.Token });
                    }
                    catch
                    {
