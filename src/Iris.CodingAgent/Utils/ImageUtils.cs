...
public static class ImageProcessor
{
    ...
    {
        if (!content.Any(b => b is Iris.Ai.ImageContent)) return content;
        var normalized = new List<Iris.Ai.ContentBlock>();
        var changed = false;
        foreach (var block in content)
        {
            if (block is not Iris.Ai.ImageContent image)
            {
            ...
            normalized.Add(new Iris.Ai.ImageContent(processed.Data, processed.MimeType));
            if (processed.Hints.Count > 0) normalized.Add(new Iris.Ai.TextContent(string.Join("\n", processed.Hints)));
            changed = true;
        }
