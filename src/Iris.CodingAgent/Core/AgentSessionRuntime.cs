...
public static class AgentSessionServicesFactory
{
    ...
    public static async Task<AgentSessionServices> CreateAsync(Options options)
    {
        ...
        global::Iris.CodingAgent.Extensions.Llama.LlamaProvider.EnsureRegistered(modelRuntime);
...
public sealed class AgentSessionRuntime
{
    ...
    public async Task<(bool Cancelled, string? SelectedText)> ForkAsync(string entryId, string position = "before")
    {
        ...
        {
            if (selectedEntry is not SessionMessageEntry { Message: Iris.Ai.UserMessage user }) throw new InvalidOperationException("Invalid entry ID for forking");
            targetLeafId = selectedEntry.ParentId;
            selectedText = user.Content.Text ?? string.Concat((user.Content.Blocks ?? []).OfType<Iris.Ai.TextContent>().Select(t => t.Text));
        }
