    ...
    private sealed partial class TreeList : IInputComponent
    {
        ...
        private static string FormatToolCall(string name, JsonObject args)
        {
            ...
            static string Js(JsonNode? node) => node switch
            {
                ...
                JsonValue v when v.TryGetValue<double>(out var d) => Iris.CodingAgent.Core.Tools.NodeCompat.FormatNumber(d),
            ...
            switch (name)
            {
                case "read":
                {
                    ...
                    if (args["offset"] is not null || args["limit"] is not null)
                    {
                        ...
                        var endText = args["limit"] is JsonValue lv && lv.TryGetValue<double>(out var l) ? Iris.CodingAgent.Core.Tools.NodeCompat.FormatNumber(start + l - 1) : "";
                        display += $":{Iris.CodingAgent.Core.Tools.NodeCompat.FormatNumber(start)}{(endText.Length > 0 ? $"-{endText}" : "")}";
                    }
                    return $"[read: {display}]";
                }
                case "write":
