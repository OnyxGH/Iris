...
public static class BuiltinCatalog
{
    ...
    private const string ResourcePrefix = "PiSharp.Ai.Providers.Data.";
...
public static class BuiltinProviders
{
    ...
    private static OAuthAuth NotYetSupportedOAuth(string name, bool isSubscription = false, string? loginLabel = null) => new()
    {
        ...
        Login = _ => throw new NotSupportedException($"{name} login is not yet supported by PiSharp"),
        Refresh = (_, _) => throw new NotSupportedException($"{name} token refresh is not yet supported by PiSharp"),
